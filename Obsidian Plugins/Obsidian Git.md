...
ssh로 ios 문제는 해결했다.
이제 좀 그만하자.. 커서 알아서 보는 걸 없애봤다. 이번엔 되면 좋겠다 진짜로
이젠 된 거일까? 근데 왜 자꾸 에러가 한 번 씩 뜨지? 아 드디어 됐다.
.git의 캐시가 문제였던 거 같다. 이건 .gitignore에 파일을 등록해도 changes에 자꾸 등록 되는 게 왜 그러는지 좀 더 알아봐야 할 거 같다.
